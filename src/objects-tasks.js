...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    const changeNeeded = bill - ticketCost;
    if (changeBank >= changeNeeded) {
      canSell = true;
      changeBank = changeBank - changeNeeded + bill;
    } else {
      canSell = false;
    }
  });
...
function fromJSON(proto, json) {
  ...
  const result = Object.assign(Object.create(proto), obj);
  return result;
}
...
class Builder {
  ...
  pseudoElement(value) {
    ...
    this.checkRepeat('pseudoElement');
    this.result += `::${value}`;
    return this;
  }
  ...
  checkRepeat(selector) {
    if (this.usedSelectors[selector]) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    this.usedSelectors[selector] = true;
    return this;
  }

  checkOrder(selector) {
    const currentOrder = this.orderMap[selector];
    if (
      this.order.length > 0 &&
      currentOrder < this.order[this.order.length - 1]
    ) {
      throw new Error(
